...
- **Plugin references** — `<config-root>/plugins/briefing.user-context.md` for section toggles, sort defaults, placeholder hints. Read-only.
- **Growth brief** — `<config-root>/relationships/today.json` for the current relationship-action queue. Read-only.
...
- **Plugin user-context** — `<config-root>/plugins/briefing.user-context.md` (after `/setup-brief`).
...
- **Dismissed log** — `<config-root>/plugins/briefing.dismissed-log.md` (append-only). One line per dismissed annotation; first 80 chars of annotation text plus item ID.
...
- **Run scheduled tasks.** Only registers them via ops `/register-schedules` if the user opts in during setup; execution is owned by Cowork's scheduled-tasks system.
...
- Plugin user-context at `<config-root>/plugins/briefing.user-context.md` (your machine).
- Dismissed log at `<config-root>/plugins/briefing.dismissed-log.md` (your machine, append-only).

## Privacy considerations

- The markdown snapshot contains email snippets, contact names, and CRM task titles. If you sync `<config-root>` to a git repo, gitignore `briefs/` and `plugins/briefing.*` files.
