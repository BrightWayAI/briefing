...
Resolve `<config-root>` (explicit override -> `CORTEX_CONFIG_ROOT` -> `~/.cortex/config-root` -> legacy pointer -> default; see `commands/brief.md` Step A for the full precedence chain), then check whether
